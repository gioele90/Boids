...
,v_limits=[0, -20.0, 10.0, 20.0],alert_distance=100,flocking_distance=10000,flocking_strength=0.125,middle_strength=0.01,frames=50,interval=50,xlim=(-500,1000),ylim=(-500,1000)):
		...
		self.xlim=xlim
		self.ylim=ylim
		self.positions=self.newflock(boids_count,np.array(p_limits[0:2])
,np.array(v_limits[2:4]))
		self.velocities=self.newflock(boids_count,np.array(v_limits[0:2]),np.array(v_limits[2:4]))
